...
- Update payloads are generated by `scripts/updates/prepare-client-update.mjs`, which copies `dist/` into `build/client-updates/builds/<buildId>/` and writes SHA-verified file metadata.
...
- A repo-local pre-commit hook (`.githooks/pre-commit`) runs `npm run build`, packages update payloads, and stages `build/client-updates/`.
